...
Waveform modification is accomplished with the help of two special waveforms. First, `Marked` waveforms provide a way of substituting one waveform for part of another. For example, to change the frequency of a sine wave, we can use a marked waveform for the first parameter:
...
```
The second waveform used in modification is the `Prior` placeholder waveform. `Prior` should never appear in a waveform during generation but can be used as an argument to the substitution process. When it does, it is replaced with the original waveform (which itself is being replaced). For example, here's a modification that reduces the amplitude of a waveform.
```
         Marked(TopLevel, Sine(Const(1643.9)), Const(0.0))
     — substitute Multiply(Prior, Const(0.5)) for "TopLevel" -->
Marked(TopLevel, Multiply(Sine(Const(1643.9)), Const(0.0)), Const(0.5))
```

Modification is different than wholesale replacement because the parts of the waveform that are not changed maintain their state. In the examples using `Sine` above, the [accumulator](sine.md#accumulation) of the `Sine` waveform will persist, limiting audible artifacts that might otherwise occur.
...
When creating new sounds or programming synthesizer components, it's often helpful to have real-time feedback on the effect of a parameter. For example, finding the best cutoff for a low-pass filter is often best done by adjusting the cutoff and listening to the result. Tuun provides a mechanism called "sliders" that supports this kind of real-time parameter tuning.
...
```
("TopLevel" is a mark used by the native app that wraps every top-level waveform.)

However, this simple implementation can lead to a "pop" sound if the amplitude of the waveform is large. Instead, "stop" is implemented with a short ramp.
```
Modify {
    mark_id: TopLevel
    waveform:
...
```
Notice the use of the `Prior` waveform here. The original waveform will be moved inside of the ramp with all of its state preserved.


## MIDI note-on and note-off

Handling MIDI note-on and note-off events is similar. MIDI instruments in Tuun are defined by a pair of functions, `(note_on, note_off)`, where these functions have the following types.

* `note_on: (midi_note, velocity) -> waveform`
* `note_off: (midi_note) -> waveform`

When a note-on event is received from a controller, the note (or key) number and the velocity (as a value between 0.0 and 1.0) are passed to the `note_on` function. The resulting waveform is played immediately. This waveform may be infinite or finite. In either case, when a note-off event is received, the `note_off` function is invoked and the original waveform is replaced by the result. This behaves similar to the way that waveforms are stopped: a simple `Fixed([])` would work, but other waveforms can be used to produce instrument-specific results.
...
```
(fn(key, vel) =>
  {map(over(@key), [1.00, 3.92, 9.24, 16.27, 24.22, 33.54, 42.97])}
     * vel | ADSR(0, 0.1, 0.3, 3.0, 2.0),
 fn(key) => prior | R(0.5, 1.0))
```
